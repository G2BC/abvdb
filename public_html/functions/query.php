...
/*
	...
	function generate($attr){
		...
		if(!empty($attr['product'])){
			...
			$query = $query."  sequencias.product LIKE '%".$attr['product']."%' ";
		...
		if(!empty($attr['host'])){
			...
			$query = $query."  species.specie LIKE '%".$attr['host']."%'";
			
		}
		
		
		//------------------------
		if(!empty($attr['continent'])){
			if($contador == 0){
					$query =$query." where";
					$contador++; 					
				}else{
			...
			$query = $query."  geodata.continent ='".$attr['continent']."'";
			
		}
		
		if(!empty($attr['sub_continent'])){
			if($contador == 0){
					$query =$query." where";
					$contador++; 					
				}else{
			...
			$query = $query."  geodata.sub_continent ='".$attr['sub_continent']."'";
